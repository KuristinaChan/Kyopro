...
void solve(){
     ...
     cin>>t;
     s+="..";
     //Use of Dijkstra on strings
     t+="..";
     map<string,ll> dis;
     pqueue<pair<pll,string>> pq;
     pq.push({{0,n},s});
     map<string,bool> vis;
     while(!pq.empty()){
        auto [c,d] = pq.top();
        ...
        for(ll i=0;i<n+1;i++){
            debug(dis);
            string tmp = d;
            if(i==p || i==p+1 || i==p-1)continue;
            tmp[p]=d[i];
            tmp[p+1] = d[i+1];
            tmp[i] = tmp[i+1] = '.';
            if(dis.count(tmp) == 0)pq.push({{c.ff-1,i},tmp});
            else {
                if(dis[tmp]>dis[d]+1)pq.push({{c.ff-1,i},tmp});
            }
        }
