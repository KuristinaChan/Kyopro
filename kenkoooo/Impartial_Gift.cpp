...
void solve(){
     ...
     rep(i,0,m){
        ...
        int upper_limit = upper_bound(all(a),b[i]+d)-a.begin();
        upper_limit--;
        debug(i,upper_limit);
        if(upper_limit==-1)continue;
        int lower_limit = lower_bound(all(a),b[i]-d)-a.begin();
        debug(i,lower_limit);
        if(upper_limit<lower_limit)continue;
        debug(i,a[upper_limit],a[lower_limit]);
        ans = max(ans,a[upper_limit]+b[i]); 
